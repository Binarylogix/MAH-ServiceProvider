...
const AllServices1 = ({ route }) => {
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      ...
      formData.append('serviceCategory', categoryNameInput);
      formData.append('vendorId', vendorId);

      if (categoryImg) {
        formData.append('img', {
          uri: categoryImg.uri,
          type: categoryImg.type || 'image/jpeg',
          name: categoryImg.fileName || 'category.jpg',
        });
      }
          ...
          <View style={styles.modalContent}>
            ...
            >
              <Text style={styles.imagePickerBtnText}>
                {categoryImg ? 'Change Image' : 'Pick Image'}
              </Text>
            </TouchableOpacity>

            {/* Display Selected or Existing Image */}
            {(categoryImg || updatedCategoryImage) && (
              <View style={styles.imagePreviewWrapper}>
                <Image
                  source={{
                    uri: categoryImg
                      ? categoryImg.uri
                      : `https://www.makeahabit.com/api/v1/uploads/ServiceCategory/${updatedCategoryImage}`,
                  }}
